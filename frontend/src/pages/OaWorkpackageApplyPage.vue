...
import {
  findWorkpackageEntries,
  findWorkpackageFormId,
  findWorkpackageTotals,
  formatPersonDays,
} from '@/utils/oaWorkpackage'
...
const labels = formFieldLabels.workpackage

const applicationNo = computed(() => {
  const receipt = task.value?.steps.find((s) => s.tool === 'workpackage_fill')?.result?.receipt_id
  if (typeof receipt === 'string' && receipt) return receipt
  return `WP-${taskId.value.replace(/^task_/, '').toUpperCase()}`
})

const totals = computed(() => {
  if (!task.value) return { totalHours: 0, totalPersonDays: 0 }
  return findWorkpackageTotals(task.value)
})
...
function periodLabel(entry: TimesheetEntry): string {
  if (entry.period && entry.period !== '全天') return entry.period
  if (entry.person_days < 1) return '半天'
  return '全天'
}
...
async function load() {
  ...
  try {
    ...
    entries.value = findWorkpackageEntries(taskRes.data)

    const formId = findWorkpackageFormId(taskRes.data)
    if (!formId) {
      error.value = '未找到关联的工时填报表单'
      return
    }
...
async function handlePrimaryClick() {
  ...
  try {
    if (phase.value === 'draft') {
      ...
      toastMessage.value = `工时填报 ${applicationNo.value} 已进入 OA 审批流程（演示）。再次点击灰色按钮可模拟审批通过。`
      notifyAssistantOaUpdate({
  ...
  <div class="oa-shell">
    <header class="oa-topbar">
      <div class="oa-brand">
        ...
        <div>
          <div class="oa-title">国能集团 · 工时报审系统</div>
          <div class="oa-subtitle">China Energy Timesheet Management · OA Module</div>
        </div>
      </div>
      <div class="oa-user">
    ...
    <nav class="oa-nav">
      ...
      <span class="nav-item">工时管理</span>
      <span class="nav-sep">/</span>
      <span class="nav-item active">工时填报</span>
    </nav>
          ...
          <table class="oa-table">
            <tbody>
              ...
              <tr>
                ...
                <td colspan="3">{{ fields.project || '—' }}</td>
              </tr>
              <tr>
                <th>{{ labels.period }}</th>
                <td>{{ fields.period || '—' }}</td>
                <th>合计工时</th>
                <td>{{ totals.totalHours }} 小时（{{ formatPersonDays(totals.totalPersonDays) }}）</td>
              </tr>
              <tr>
                <th>{{ labels.content }}</th>
                <td colspan="3">{{ fields.content || '—' }}</td>
              </tr>
            </tbody>
          </table>

          <div class="section-title">二、每日工时明细</div>
          <table v-if="entries.length" class="oa-table booking-table">
            <thead>
              <tr>
                <th>日期</th>
                <th>星期</th>
            ...
            <tbody>
              <tr v-for="entry in entries" :key="entry.day_date">
                <td>{{ formatOaDate(entry.day_date) }}</td>
                <td>{{ entry.day_label || '—' }}</td>
                <td>{{ periodLabel(entry) }}</td>
                <td>{{ entry.hours }}</td>
                <td>{{ entry.person_days }}</td>
              </tr>
            </tbody>
          </table>
          <p v-else class="empty-hint">暂无明细数据，请返回助手重新生成填报计划。</p>
