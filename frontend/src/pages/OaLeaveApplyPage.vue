...
const leaveDetail = ref<ReturnType<typeof findLeaveApplyResult>>({})
const phase = ref<OaPhase>('draft')
const submitting = ref(false)
const toastMessage = ref<string | null>(null)

const taskId = computed(() => String(route.params.taskId ?? ''))
const labels = formFieldLabels.leave

const applicationNo = computed(() => {
  const receipt = task.value?.steps.find((s) => s.tool === 'leave_apply')?.result?.receipt_id
  if (typeof receipt === 'string' && receipt) return receipt
  return `LA-${taskId.value.replace(/^task_/, '').toUpperCase()}`
})

const periodDisplay = computed(() => {
  const start = fields.value.date_start || leaveDetail.value.date_start
  const end = fields.value.date_end || leaveDetail.value.date_end
  if (!start || !end) return '—'
  if (start === end) {
    const period = fields.value.start_period || leaveDetail.value.start_period
    const dateLabel = formatOaDate(start)
    return period && period !== '全天' ? `${dateLabel}（${period}）` : dateLabel
  }
  return `${formatOaDate(start)} 至 ${formatOaDate(end)}`
})

const attachmentName = computed(
  () => fields.value.attachment_name || leaveDetail.value.attachment_name || '',
)

const approvalChain = computed(() =>
  buildLeaveApprovalChain(auth.user?.display_name ?? '—', phase.value),
)
...
async function load() {
  ...
  try {
    ...
    leaveDetail.value = findLeaveApplyResult(taskRes.data)

    const formId = findLeaveFormId(taskRes.data)
    if (!formId) {
      error.value = '未找到关联的请假申请表单'
      return
    }
...
async function handlePrimaryClick() {
  ...
  try {
    if (phase.value === 'draft') {
      ...
      toastMessage.value = `请假申请 ${applicationNo.value} 已进入 OA 审批流程（演示）。再次点击灰色按钮可模拟审批通过。`
  ...
  <div class="oa-shell">
    <header class="oa-topbar">
      <div class="oa-brand">
        ...
        <div>
          <div class="oa-title">国能集团 · 请假管理系统</div>
          <div class="oa-subtitle">China Energy Leave Management · OA Module</div>
        </div>
      </div>
      <div class="oa-user">
    ...
    <nav class="oa-nav">
      ...
      <span class="nav-item">人事管理</span>
      <span class="nav-sep">/</span>
      <span class="nav-item active">请假申请</span>
    </nav>
    ...
    <main class="oa-main">
      ...
      <template v-else>
        ...
        <section class="oa-card">
          ...
          <table class="oa-table">
            <tbody>
              ...
              <tr>
                ...
                <td>智能矿山事业部</td>
                <th>申请日期</th>
                <td>{{ formatOaDate(new Date().toISOString()) }}</td>
              </tr>
              <tr>
                <th>{{ labels.leave_type }}</th>
                <td>{{ fields.leave_type || leaveDetail.leave_type || '—' }}</td>
                <th>{{ labels.days }}</th>
                <td>{{ fields.days || leaveDetail.days || '—' }} 天</td>
              </tr>
            </tbody>
          </table>

          <div class="section-title">二、请假详情</div>
          <table class="oa-table">
            <tbody>
              <tr>
                <th>请假时间</th>
                <td colspan="3">{{ periodDisplay }}</td>
              </tr>
              <tr>
                <th>{{ labels.reason }}</th>
                <td colspan="3">{{ fields.reason || leaveDetail.reason || '—' }}</td>
              </tr>
              <tr>
                <th>附件</th>
                <td colspan="3">
                  <span v-if="attachmentName" class="attachment-name">{{ attachmentName }}</span>
          ...
          <div class="section-title">三、审批流程</div>
